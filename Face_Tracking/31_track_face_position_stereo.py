...
def full_code():
    ...
    while cam1.isOpened() and cam2.isOpened():
        ...
        rectify_scale = 1
        R1, R2, P1, P2, Q, _, _ = cv2.stereoRectify(camera_matrix_right, camera_distortion_right, camera_matrix_left, camera_distortion_left, image1g.shape[::-1], rotation_matrix, translation_vector,rectify_scale,(0,0))

        if success1 == True:
            ret_right, nose_right=check_face_orientation(1, image1,face_mesh1,mp_face_mesh1)
        if success2 == True:
            ret_left, nose_left=check_face_orientation(0, image2,face_mesh2,mp_face_mesh2)
