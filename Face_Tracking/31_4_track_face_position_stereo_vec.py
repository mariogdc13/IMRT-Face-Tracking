...
def full_code():
    # camera parameters
    camL_alpha = 58  # [degrees]
    camR_alpha = 58  # [degrees]
    camL_focal = 2.3  # [mm]
    camR_focal = 2.3  # [mm]
    cam_baseline = 9.2  # [cm] distance between cameras
    cam_depth_multiplier = 12.26  # found manually

    #load stereo calibration data
    stereo_calibration_data = np.load('calibration_files\camera_stereo_calibration_data.npz')

    camera_matrix_right = stereo_calibration_data['Kr']
    camera_distortion_right = stereo_calibration_data['Dr']
    camera_matrix_left = stereo_calibration_data['Kl']
    camera_distortion_left = stereo_calibration_data['Dl']
    rotation_matrix = stereo_calibration_data['R']
    translation_vector = stereo_calibration_data['T']
    #translation_vector = np.array([-100,-0.26755334 ,0])
    essential_matrix = stereo_calibration_data['R']
    fundamental_matrix = stereo_calibration_data['F']

    #load stereo rectification data
    stereo_rectification_data = np.load('calibration_files\camera_stereo_rectification_data.npz')

    rectification_L = stereo_rectification_data['rectification_L']
    rectification_R = stereo_rectification_data['rectification_R']
    projMatrixL = stereo_rectification_data['projMatrixL']
    projMatrixR = stereo_rectification_data['projMatrixR']
    Q = stereo_rectification_data['Q']
    roi_l = stereo_rectification_data['roi_l']
    roi_r = stereo_rectification_data['roi_r']
    stereoMapL_x = stereo_rectification_data['stereoMapL_x']
    stereoMapL_y = stereo_rectification_data['stereoMapL_y']
    stereoMapR_x = stereo_rectification_data['stereoMapR_x']
    stereoMapR_y = stereo_rectification_data['stereoMapR_y']

    # Camera setup
    cam_r = cv2.VideoCapture(1)  # right camera
    cam_l = cv2.VideoCapture(0)  # left camera
    ...
    while cam_r.isOpened() and cam_l.isOpened():

        success_r, image_r = cam_r.read()  # adquire frame
        success_l, image_l = cam_l.read()  # adquire frame

        image_r_gray = cv2.cvtColor(image_r, cv2.COLOR_BGR2GRAY)
        image_l_gray = cv2.cvtColor(image_l, cv2.COLOR_BGR2GRAY)

        #rectify_scale = 1
        #R1, R2, P1, P2, Q, _, _ = cv2.stereoRectify(camera_matrix_right, camera_distortion_right, camera_matrix_left, camera_distortion_left, image_r_gray.shape[::-1], rotation_matrix, translation_vector,rectify_scale,(0,0))

        image_r_rectifiyed = cv2.remap(image_r, stereoMapR_x, stereoMapR_y, cv2.INTER_LANCZOS4, cv2.BORDER_CONSTANT,0)
        image_l_rectifiyed = cv2.remap(image_l, stereoMapL_x, stereoMapL_y, cv2.INTER_LANCZOS4, cv2.BORDER_CONSTANT, 0)

        if success_r == True:
           ret_right, nose_right, l_eye_right, r_eye_right=check_face_orientation(1, image_r,face_mesh1,mp_face_mesh1)
        #   ret_right, nose_right = check_face_orientation(1, image_r_rectifiyed, face_mesh1, mp_face_mesh1)
        if success_l == True:
           ret_left, nose_left, l_eye_left, r_eye_left=check_face_orientation(0, image_l,face_mesh2,mp_face_mesh2)
        #   ret_left, nose_left = check_face_orientation(0, image_l_rectifiyed, face_mesh2, mp_face_mesh2)
        ...
        if ret_right and ret_left:
            ...
            points4D = cv2.triangulatePoints(projMatrixL, projMatrixR, np.array([[nose_right[0]], [nose_right[1]]]),
                                             np.array([[nose_left[0]], [nose_left[1]]]))
            l_eye_points4D = cv2.triangulatePoints(projMatrixL, projMatrixR, np.array([[l_eye_right[0]], [l_eye_right[1]]]),
                                                   np.array([[l_eye_left[0]], [l_eye_left[1]]]))
            r_eye_points4D = cv2.triangulatePoints(projMatrixL, projMatrixR, np.array([[r_eye_right[0]], [r_eye_right[1]]]),
            ...
            eye_vector = (r_eye_points3D[0] - l_eye_points3D[0], r_eye_points3D[1] - l_eye_points3D[1], r_eye_points3D[2] - l_eye_points3D[2])


            angles_x, angle_y, angle_z = vector_angles(eye_vector)

            depth_face = stereovision_find_depth(nose_left, nose_right, image_l, image_r, camR_focal,
                                                 camR_alpha, cam_baseline)
            depth_face = depth_face * cam_depth_multiplier
            print(f'nose position: {depth_face}')
            #z_depth=compute_depth(nose_left[0], nose_right[0], 1128, translation_vector[0])

            # Print the real-world coordinates
            print("3D Coordinates in real-world space: ", points3D.flatten())
            print("Coordenades ull esquerra: ", l_eye_points3D.flatten())
            print("Coordenades ull dret: ", r_eye_points3D.flatten())
            print("Angle cap: ", angles_x, " ", angle_y, " ", angle_z)
