...
CHECKERBOARD = (10 , 7)  # (number of internal corners per a chessboard row, number of internal corners per a chessboard column)
square_size = 24  # This is the real world size of a square in your checkerboard (e.g., 1.0 if squares are 1x1 units)

# Directory containing checkerboard images
image_directory = 'calibration_images\single_right'
...
op_mtx, roi = cv2.getOptimalNewCameraMatrix(mtx,dist,gray.shape[::-1],1,gray.shape[::-1])
...
calibration_data = {
    ...
    'rvecs': rvecs,
    'tvecs': tvecs,
    'optimal_matrix':op_mtx,
    'roi':roi
}

np.savez('calibration_files\camera_right_calibration_data.npz', **calibration_data)
